...
// A modified version of the ProfilePage that doesn't have its own AppHeader and AppSidebar
...
class _ProfilePageContentState extends State<ProfilePageContent> {
  ...
  Future<void> _loadUserInfo() async {
    if (!mounted) {
      print("ProfilePageContent: Widget not mounted during _loadUserInfo, returning early");
      return;
    }
    ...
    try {
      ...
      if (user == null) {
        if (!mounted) {
          print("ProfilePageContent: Widget not mounted after user check, returning early");
          return;
        }
        setState(() {
          _isLoading = false;
      ...
      if (!mounted) {
        print("ProfilePageContent: Widget not mounted after API call, returning early");
        return;
      }
      ...
      if (response.statusCode == 200 && response.body.isNotEmpty) {
        ...
        if (!mounted) {
          print("ProfilePageContent: Widget not mounted after JSON decode, returning early");
          return;
        }
        setState(() {
  ...
  Future<void> _saveProfile() async {
    ...
    if (!mounted) {
      print("ProfilePageContent: Widget not mounted during _saveProfile, returning early");
      return;
    }
    ...
    try {
      ...
      if (user == null) {
        if (!mounted) {
          print("ProfilePageContent: Widget not mounted after user check in _saveProfile, returning early");
          return;
        }
        setState(() {
          _isSaving = false;
      ...
      if (!mounted) {
        print("ProfilePageContent: Widget not mounted after API call in _saveProfile, returning early");
        return;
      }
  ...
  Widget build(BuildContext context) {
    // Return just the content part without the Scaffold, AppHeader, and AppSidebar
    return _isLoading
